...
namespace BlockadeLabsSDK
{
    public class BlockadeLabsSkybox : MonoBehaviour
    {
        ...
        public async Task GetAssets()
        {
            ...
            await DownloadTexture(textureUrl, prompt, false);
            await DownloadTexture(depthMapUrl, prompt, true);
        ...
        private async Task DownloadTexture(string url, string prompt, bool isDepth)
        {
            if (!string.IsNullOrWhiteSpace(url))
            {
                var image = await ApiRequests.GetImagineImage(url);
                ...
                if (assignToMaterial)
                {
                    ...
                    if (r != null)
                    {
                        ...
                        {
                            r.sharedMaterial.SetTexture(isDepth ? "_Depth" : "_MainTex", texture);
                        }
                    }
                }
                ...
                CalculateProgress();
                SaveAssets(texture, prompt, isDepth);
            }
        }

        private void SaveAssets(Texture2D texture, string prompt, bool isDepth)
        {
            #if UNITY_EDITOR
                ...
                var textureName = ValidateFilename(prompt) + (isDepth ? "_depth" : "_texture");
